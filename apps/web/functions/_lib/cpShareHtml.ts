...
import { escapeJsonForScript } from './cpShare';
...
// The `<head>` surgery, shared with the prerender — the two edit the same template and
// must not learn to disagree about it.
import { setDocumentTitle, setMetaTag, stripJsonLd, verbatim } from '../../src/seo/htmlMeta';
...
export function renderShareCardMeta(html: string, meta: ShareCardMeta): string {
  ...
  // Same reasoning as `stripSeoContent`, one field over: every share serves
  // `dist/index.html`, so without this a share page publishes the landing's graph — a
  // `WebSite` node claiming to be the site, and a `SoftwareApplication` describing Ori
  // Studio — under a card that describes someone's crease pattern.
  let next = stripJsonLd(stripSeoContent(html));
