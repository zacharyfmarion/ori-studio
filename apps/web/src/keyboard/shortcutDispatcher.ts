...
export interface ShortcutExecutors {
  ...
  /**
   * Returning `false` declines the chord: the viewport had a binding for it but
   * nothing to do, so dispatch continues to the next scope. Anything else
   * (including `undefined`) claims it.
   *
   * This is what lets a viewport bind a chord a broader scope also owns. Delete
   * is the case it exists for -- the viewport deletes the selected canvas object
   * when there is one, and otherwise hands the key back to `edit.delete`, which
   * deletes creases. Without a decline the viewport binding would shadow crease
   * deletion outright, which is why that verb used to live in a raw `keydown`
   * listener on the panel instead.
   *
   ...
   */
  viewport?: (id: ViewportShortcutId) => boolean | void;
  /**
...
function executeShortcut(
  ...
  switch (target) {
    ...
    case 'viewport':
      ...
      return executors.viewport(id as ViewportShortcutId) !== false;
    case 'simulator':
