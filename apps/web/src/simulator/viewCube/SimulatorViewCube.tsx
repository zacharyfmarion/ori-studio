 ...
 * that must not be 60 React renders.
 */
...
export interface SimulatorViewCubeProps {
  ...
  /** The camera at mount. Later changes arrive through the handle. */
  initialView: SimulatorOrbitView;
...
export function SimulatorViewCube({
  ...
  // Captured once, like the viewport's own opening view: a caller rebuilding
  // this object every render must not turn the cube back mid-gesture.
  const openingViewRef = useRef(initialView);
  ...
  // The opening pose, before any camera has been pushed. Without it the cube
  // would sit unrotated until the first orbit.
  useEffect(() => {
      ...
      <div ref={sceneRef} className="simulator-view-cube__scene">
        {VIEW_CUBE_FACES.map((face, index) => (
          <button
            ...
            onClick={() => onSnap(face.direction, face.id)}
          >
            {faceLabel(t, face.id)}
          </button>
        ))}
