/**
 * What one step of a ReferenceFinder construction is *about*, on the CP view.
 *
 ...
 * worker.
 *
 * The view no longer *draws* from this summary: it draws the card's own
 * diagram, mapped onto the pattern (`referenceFinderStepInModel`), so the two
 * pictures of a step are one description. What is left of the summary's job
 * is the frame — `bounds`, the step's references and what it makes, for the
 * camera to show.
 */
