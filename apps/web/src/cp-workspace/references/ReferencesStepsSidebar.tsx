...
/**
 * The References workspace's left sidebar: ReferenceFinder's ranked answers
 * for the picked target as cards, and the selected candidate's steps as a list
 * of sentences with a small diagram each. With no target it says what to do.
 *
...
export const ReferencesStepsSidebar = memo(function ReferencesStepsSidebar({
  ...
  const showCandidates = target !== null && candidates !== null && candidates.length > 0;
  ...
  return (
    ...
    >
      <div className="references-sidebar__header">
        <ListOrdered size={14} />
        <span className="panel-title">{t('panels:references.sidebar.title', 'Steps')}</span>
        {showCandidates && (
          <span className="references-sidebar__count">{candidates.length}</span>
        )}
      </div>
        ...
        <div className="references-sidebar__hint">
          <p>
            {target && status === 'running'
              ...
              : target && status === 'idle' && candidates?.length === 0
                ? t(
