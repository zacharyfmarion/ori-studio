    ...
    switch (input.kind) {
      case 'down':
        ...
        return state.armed && state.start
          ? { state, preview: segment(state.start, input.point), commit: null }
          : { state: { start: input.point, armed: false }, preview: null, commit: null };
      ...
      case 'move':
        ...
        return { state, preview: segment(state.start, input.point), commit: null };

      case 'up': {
        const start = state.start;
        if (!start) return { state: IDLE, preview: null, commit: null };
        if (beyondTolerance(start, input.point, input.tolerance)) {
          return { state: IDLE, preview: null, commit: { points: [start, input.point] } };
        }
        ...
        return state.armed
          ? { state: IDLE, preview: null, commit: null }
          : { state: { start, armed: true }, preview: null, commit: null };
      }
