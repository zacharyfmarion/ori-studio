...
const COMPLETIONS: CpSolveCompletion[] = ['exact', 'approximate', 'improved', 'unfoldable'];

function residuals(over: Partial<CpExactSolveResiduals> = {}): CpExactSolveResiduals {
  return {
...
describe('cpSolveCompletion', () => {
  ...
  it('reads an ambiguous solve over clean topology as improved, not solved', () => {
    expect(cpSolveCompletion(accepted('ambiguous', residuals()))).toBe('improved');
  });
  ...
  it('separates the solver at 1e-3 from the check at 1e-6', () => {
    ...
    // sees flagged.
    const inWindow = accepted('solved', residuals({ maxKawasakiDegreesAfter: 5e-4 }));
...
describe('the two predicates', () => {
  ...
  it('keeps `approximate` on the solver-accepted side of the button gate', () => {
    ...
    // inventing work.
    expect(COMPLETIONS.filter(cpSolveIsExactVerdict)).toEqual(['exact', 'approximate']);
...
describe('formatSolveAngleDegrees', () => {
  ...
  it('spells the check bar out rather than in exponent notation', () => {
    expect(formatSolveAngleDegrees(CP_FOLDABILITY_CHECK_EPSILON_DEGREES)).toBe('0.000001');
...
describe('cpSolveCompletionHeadline', () => {
  it('gives each ending its own sentence, and only one of them says Solved alone', () => {
    const headlines = COMPLETIONS.map((completion) => cpSolveCompletionHeadline(t, completion));
...
describe('cpSolveCompletionDetail', () => {
  ...
  it('leads with the odd-degree cause, because that is the actionable one', () => {
    ...
    expect(detail.startsWith('3 vertices still have an odd number of creases')).toBe(true);
  ...
  it('defers to the shared table when there are no numbers to quote', () => {
    ...
    const detail = cpSolveCompletionDetail(t, { completion: 'improved', residuals: null });
    expect(detail).toContain('not close enough for the pattern to pass the foldability check');
  });

  it('is the only ending allowed to say the check passes', () => {
    const exact = cpSolveCompletionDetail(t, { completion: 'exact', residuals: residuals() });
    expect(exact).toBe('The pattern now meets the foldability check.');
    for (const completion of ['approximate', 'improved', 'unfoldable'] as const) {
      const detail = cpSolveCompletionDetail(t, { completion, residuals: residuals() });
...
describe('mid-solve_2.osf, end to end from the solver payload', () => {
  function midSolve2(): CpExactSolvedGraph {
    return {
      ...
      theorem_residual_report: {
        ...
        accepted: true,
        before: { max_kawasaki_residual_degrees: 14.367, odd_degree_vertices: [17, 42, 88] },
        after: { max_kawasaki_residual_degrees: 0.00747, odd_degree_vertices: [17, 42, 88] },
      },
  ...
  it('reports a refused polish round as not adopted, from the count', () => {
    ...
    const outcome = classifyCpExactSolve(midSolve2(), 'refinement');
    expect(isCpExactSolveAccepted(outcome) && outcome.polishAdopted).toBe(false);
  });

  it('lands on unfoldable, and never says Solved', () => {
    const outcome = classifyCpExactSolve(midSolve2(), 'refinement');
    if (!isCpExactSolveAccepted(outcome)) throw new Error('expected an accepted outcome');
