...
describe('oristudioBpProjectStateFromRaw', () => {
  ...
  it('maps wasm crease pattern snapshots into BP crease pattern views', () => {
    const state = oristudioBpProjectStateFromRaw({
      handle: 42,
      project: rawProject,
    ...
    expect(state.snapshot.creasePattern).toMatchObject({
      sheet: { kind: 'rectangular', width: 8, height: 8 },
      stale: false,
    });
    expect(state.snapshot.creasePattern?.lines).toEqual([
      expect.objectContaining({
        id: 'bp-cp-0',
        assignment: 'border',
        sourceLayer: 'sheet-border',
      }),
      expect.objectContaining({
        id: 'bp-cp-1',
        assignment: 'mountain',
