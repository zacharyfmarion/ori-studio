...
describe("classifyCpExactSolve", () => {
  ...
  it("does not call an accepted-but-ambiguous run solved", () => {
    ...
    const outcome = classifyCpExactSolve(
      graph(
        { timed_out: false, accepted: true, rejection_reasons: [] },
        "ambiguous",
      ),
      "refinement",
    );
  ...
  it("drops a token it has no sentence for rather than surfacing it raw", () => {
    ...
    expect(outcome.kind === "rejected" && outcome.reasons).toEqual([
  ...
  it("offers the partial from attempted_moved_vertices, not from moved_vertices", () => {
    ...
    expect(
      outcome.kind === "timeout" && outcome.partialMovedVertices,
    ).toHaveLength(2);
  });
  ...
  it("does not mistake a malformed run for an untimed rejection", () => {
    ...
    const outcome = classifyCpExactSolve(
      graph({ status: "not_run", blockers: ["x"] }),
      "geometry",
    );
...
describe("cpExactSolveResiduals", () => {
  function report(theorem: CpExactSolveTheoremReport) {
    return cpExactSolveResiduals(
      graph({ accepted: true }, "ambiguous", theorem),
    );
  }
  ...
  it("refuses a half-written side rather than inventing the other half", () => {
    ...
    expect(
      report({
        before: MID_SOLVE_RESIDUALS.before,
        after: { odd_degree_vertices: [] },
      }),
    ).toBeNull();
...
describe("the reason vocabulary", () => {
  ...
  it("recognises every token it lists and nothing else", () => {
    for (const reason of CP_EXACT_SOLVE_REASONS)
      expect(isCpExactSolveReason(reason)).toBe(true);
...
describe("cpExactSolveAngleFamily", () => {
  ...
  it("is null when the report has no grid block", () => {
    ...
    expect(
      cpExactSolveAngleFamily(solvedWith({ polish: { pinned_family: null } })),
    ).toBeNull();
    expect(
      cpExactSolveAngleFamily(solvedWith({ polish: { pinned_family: {} } })),
    ).toBeNull();
  });
            ...
            attempts: [
              {
                ...
                refusals: ["candidate_status_failed"],
                kawasaki_over_bar: 165,
              },
              {
                tolerance_degrees: 0.75,
                refusals: ["pinned_kawasaki_regressed"],
                kawasaki_over_bar: 33,
              },
            ],
          },
        },
      ...
      solvedWith({
        polish: {
          pinned_family: {
            ...
            stop_reason: "adopted",
            attempts: [
              { tolerance_degrees: 1.5, refusals: [], kawasaki_over_bar: 0 },
            ],
          },
        },
      }),
...
describe("cpExactSolvePleats", () => {
  ...
  it("is null when the report has no pleat block", () => {
    expect(cpExactSolvePleats(solvedWith({}))).toBeNull();
    expect(
      cpExactSolvePleats(solvedWith({ polish: { pleat_runs: null } })),
    ).toBeNull();
  });

  it("counts the runs and their creases, and reads the spreads as percentages", () => {
    const pleats = cpExactSolvePleats(
      solvedWith({
        polish: {
          pleat_runs: {
            adopted: true,
            ...
            refusals: [],
            runs: [
              { family_degrees: 0, creases: 30 },
              { family_degrees: 90, creases: 6 },
            ],
          },
        },
      }),
    );
    expect(pleats).toEqual({
      adopted: true,
      stopReason: "adopted",
      runs: 2,
      creases: 36,
      ...
      solvedWith({
        polish: {
          pleat_runs: {
            ...
            stop_reason: "refused",
            ties: 3,
            refusals: ["movement_budget_exceeded"],
            runs: [{ creases: 5 }],
          },
        },
      }),
    );
    expect(pleats?.adopted).toBe(false);
    expect(pleats?.refusals).toEqual(["movement_budget_exceeded"]);
    expect(pleats?.spreadBeforePct).toBeNull();
  });
});
