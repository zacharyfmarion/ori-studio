use super::{EventKind, LineSegment, compare_float, float_xy_cmp};
...
impl GeneralUnion {
    ...
    pub fn get(&mut self, components: &[Vec<Vec<PathPoint>>]) -> Vec<PathEx> {
        general_union(components)
    }
}

pub fn general_union(components: &[Vec<Vec<PathPoint>>]) -> Vec<PathEx> {
    GeneralUnionSweep::new(components).get()
}
...
impl EventQueue {
    ...
    fn pop(&mut self, events: &[GeneralEvent], segments: &[LineSegment]) -> Option<usize> {
        let mut best = 0;
...
impl Status {
    fn insert(&mut self, id: usize, events: &[GeneralEvent], segments: &[LineSegment]) {
    ...
    fn sort(&mut self, events: &[GeneralEvent], segments: &[LineSegment]) {
        self.ids
            .sort_by(|a, b| status_cmp(events, segments, *a, *b));
    }
}

struct GeneralUnionSweep {
...
impl GeneralUnionSweep {
    fn new(components: &[Vec<Vec<PathPoint>>]) -> Self {
        let mut sweep = Self {
            events: Vec::new(),
            segments: Vec::new(),
    ...
    fn get(mut self) -> Vec<PathEx> {
        ...
        chain_segments(&self.collected, &self.segments)
            .into_iter()
            .map(|points| {
                let is_hole = crate::math::geometry::is_clockwise(&points);
                PathEx {
                    points,
                    is_hole,
                    from: None,
    ...
    fn add_segment_delta_orientation(&mut self, segment: LineSegment, delta: i8) {
...
fn event_cmp(events: &[GeneralEvent], segments: &[LineSegment], a: usize, b: usize) -> Ordering {
        ...
        .then_with(|| {
            if a_event.kind == EventKind::Start && b_event.kind == EventKind::Start {
                segment_cmp(events, segments, a, b)
                    .then_with(|| b_event.wrap_delta.cmp(&a_event.wrap_delta))
            } else {
                Ordering::Equal
            }
        })
...
fn status_cmp(events: &[GeneralEvent], segments: &[LineSegment], a: usize, b: usize) -> Ordering {
    compare_up_down(events, a, b)
        .then_with(|| segment_cmp(events, segments, a, b))
        .then_with(|| events[b].wrap_delta.cmp(&events[a].wrap_delta))
        .then_with(|| events[a].key.cmp(&events[b].key))
}

fn segment_cmp(events: &[GeneralEvent], segments: &[LineSegment], a: usize, b: usize) -> Ordering {
...
fn compare_up_down(events: &[GeneralEvent], a: usize, b: usize) -> Ordering {
    let a_event = &events[a];
    let b_event = &events[b];
...
fn event_slope(events: &[GeneralEvent], event: usize) -> f64 {
...
fn chain_segments(segment_ids: &[usize], segments: &[LineSegment]) -> Vec<Vec<PathPoint>> {
    let mut chains: Vec<Chain> = Vec::new();
    let mut result = Vec::new();
    for segment in segment_ids.iter().map(|id| &segments[*id]) {
        let tail = chains
            .iter()
            .position(|chain| same_point(chain.points.first(), &segment.end));
        let head = chains
            .iter()
            .position(|chain| same_point(chain.points.last(), &segment.start));
...
fn same_point(actual: Option<&PathPoint>, expected: &PathPoint) -> bool {
    actual.is_some_and(|point| epsilon_same((point.x, point.y), (expected.x, expected.y), EPSILON))
}
