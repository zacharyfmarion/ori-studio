...
fn an_admitted_verdict_carries_the_residual_of_every_bar_the_gate_applied() {
    ...
    let mut checked_a_loop_with_no_vertex = false;
    for name in ADMITTED {
        ...
        if diagnostics.loop_gap_non_tree_edges > 0 {
            ...
            // Localisation is only possible where there is a vertex to localise
            // *to*. The premise — every independent dual cycle is generated by
            // an interior vertex's face ring — holds on simply connected paper
            // and fails on two ordinary documents: a holed sheet whose creases
            // all run rim to rim (`hole_vertex_90`, 1 non-tree edge and 0
            // interior vertices), and a sheet whose one crease is drawn as two
            // collinear pieces. Both are admitted correctly; what they lack is a
            // vertex, so the gate reads their gap without being able to say
            // where it came from. Gating on `spatial_vertices` is the premise
            // itself rather than a carve-out for a fixture.
            if diagnostics.spatial_vertices > 0 {
                assert!(
                    diagnostics.vertex_cycles > 0,
                    "{name}: {} non-tree edges over {} interior vertices, but no \
                     elementary vertex cycle to localise them",
                    diagnostics.loop_gap_non_tree_edges,
                    diagnostics.spatial_vertices
                );
            } else {
                assert_eq!(
                    diagnostics.vertex_cycles, 0,
                    "{name}: a vertex cycle with no interior vertex to build it from"
                );
                checked_a_loop_with_no_vertex = true;
            }
        }
    }
    assert!(
    ...
    assert!(
        checked_a_loop_with_no_vertex,
        "no fixture carries a dual cycle that no interior vertex generates, so the \
         branch above tested nothing. That shape is what makes the loop gap a \
         primary check rather than defence in depth — `hole_vertex_90` is it"
    );
}
