...
fn an_admitted_verdict_carries_the_residual_of_every_bar_the_gate_applied() {
    ...
    for name in ADMITTED {
        ...
        if diagnostics.loop_gap_non_tree_edges > 0 {
            ...
            // Only on **simply connected** paper, which every fixture in
            // `ADMITTED` is. The premise is that each independent dual cycle is
            // generated by an interior vertex's face ring, and two ordinary
            // documents break it: a holed sheet whose creases all run
            // boundary-to-boundary (`a_holed_sheet_whose_hole_cycle_closes_is_admitted`
            // measures 1 non-tree edge and 0 vertex cycles), and a sheet whose
            // one crease is drawn as two collinear pieces. Both are admitted
            // correctly; what they lack is a *vertex* to localise the gap to.
            // Widening this list to such a fixture means dropping the assertion,
            // not fixing the fixture.
            assert!(
