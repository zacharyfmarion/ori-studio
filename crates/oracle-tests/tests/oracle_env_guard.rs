...
//! environment variable is unset, so a plain `cargo test -p oracle-tests`
//! needs no C++ oracle or Node. The cost of that convenience is that an
//! unconfigured suite and a passing suite look identical in the log — which is
//! how seven of this workspace's eight oracle suites ran dormant in CI.
//!
