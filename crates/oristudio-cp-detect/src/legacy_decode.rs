...
use oristudio_cp_compiler::{
    AssignmentCandidate as CompilerAssignmentCandidate, AssignmentLabel as CompilerAssignmentLabel,
    CandidateCarrier as CompilerCandidateCarrier, CandidateEdge as CompilerCandidateEdge,
    CandidateProgram, CandidateVertex as CompilerCandidateVertex,
    CarrierFamily as CompilerCarrierFamily, EdgeSelection as CompilerEdgeSelection,
    EvidenceSource as CompilerEvidenceSource, Point2 as CompilerPoint2,
    Provenance as CompilerProvenance, VertexKind as CompilerVertexKind,
};
...
pub(crate) fn compiler_candidate_program_from_dense_outputs(
    outputs: DenseOutputs<'_>,
    config: DecodeConfig,
    legacy_fold_json: &str,
    ...
    let legacy_fold: Value = serde_json::from_str(legacy_fold_json)?;
    let mut program = CandidateProgram::from_fold_value(&legacy_fold)?;
    let evidence_config = compiler_evidence_config(&config);
    let line_prob = sigmoid_map(outputs.line_logits);
    let effective = effective_line_prob(outputs, &evidence_config);
    let (line_mask, foreground_count) = hough_mask(&effective, &evidence_config);
    if foreground_count == 0 {
        return Ok(program);
    }
    let carriers: Vec<Line> = hough_lines(&effective, &evidence_config)
        .into_iter()
    ...
    let assignment_labels = assignment_labels_from_logits(
        ...
        size,
        evidence_config.threshold,
    );
    let evidence_edges = interior_edges(
    ...
    let attributed_edges = attribute_edges_from_logits(
        &vertex_stage.merged_vertices,
        &evidence_edges,
        outputs.assignment_logits,
        &line_prob,
    ...
    append_evidence_candidates_to_program(
        &mut program,
        &vertex_stage.merged_vertices,
        &attributed_edges,
        size,
...
fn append_evidence_candidates_to_program(
    ...
    for edge in edges {
        ...
        let Some(start_px) = vertices_px.get(edge.edge.a).copied() else {
            continue;
        };
        let Some(end_px) = vertices_px.get(edge.edge.b).copied() else {
            continue;
        };
        if distance(start_px, end_px) < config.min_edge_length_px {
            continue;
        }
        let start = normalize_compiler_point(start_px, size);
        let end = normalize_compiler_point(end_px, size);
        if compiler_distance(start, end) <= 1e-9 {
            continue;
        }
        let start_index = upsert_compiler_vertex(program, start, edge.edge.support, size, config);
        let end_index = upsert_compiler_vertex(program, end, edge.edge.support, size, config);
        if start_index == end_index
            || compiler_edge_exists(program, start_index, end_index, start, end, config)
        {
            continue;
        }
        let support = f64::from(edge.edge.support.clamp(0.0, 1.0));
        let source = if edge.edge.support >= config.min_edge_support {
            CompilerEvidenceSource::ObservedStrong
        } else {
            CompilerEvidenceSource::ObservedWeak
        };
        let provenance = match source {
            CompilerEvidenceSource::ObservedStrong => vec![CompilerProvenance::ObservedStrong],
            _ => vec![CompilerProvenance::ObservedWeak],
        };
        let carrier_id = next_compiler_carrier_id(program);
        let edge_id = next_compiler_edge_id(program);
        let normal = compiler_line_normal(start, end);
        program.carriers.push(CompilerCandidateCarrier {
            id: carrier_id,
            family: compiler_carrier_family(start, end, CompilerAssignmentLabel::Unknown),
            normal,
            rho: normal.x * start.x + normal.y * start.y,
            support_interval: compiler_support_interval(start, end),
        ...
        program.edges.push(CompilerCandidateEdge {
            id: edge_id,
            carrier_id,
            vertices: [start_index, end_index],
            assignment: CompilerAssignmentCandidate {
                label: compiler_assignment_from_u8(edge.edge.assignment),
                confidence: f64::from(edge.confidence.clamp(0.0, 1.0)),
                margin: f64::from(edge.margin.clamp(0.0, 1.0)),
            },
            line_support: support,
            style_support: 0.0,
            selection: CompilerEdgeSelection::Undecided,
            source,
            provenance,
        });
    }
...
fn normalize_compiler_point(point: Point, size: usize) -> CompilerPoint2 {
    let max = (size - 1).max(1) as f64;
    CompilerPoint2::new(
        (f64::from(point.x) / max).clamp(0.0, 1.0),
        (f64::from(point.y) / max).clamp(0.0, 1.0),
    )
}

fn upsert_compiler_vertex(
    program: &mut CandidateProgram,
    point: CompilerPoint2,
    support: f32,
    size: usize,
    config: &DecodeConfig,
) -> usize {
    let tolerance = f64::from((config.vertex_merge_px + 1.0) / (size - 1).max(1) as f32);
    if let Some(index) = program
        .vertices
        .iter()
        .position(|vertex| compiler_distance(vertex.position, point) <= tolerance)
    {
        return index;
    }
    let id = program.vertices.len();
    program.vertices.push(CompilerCandidateVertex {
        id,
        position: point,
        kind: compiler_vertex_kind(point, tolerance),
        support: f64::from(support.clamp(0.0, 1.0)),
        boundary_side: compiler_boundary_side(point, tolerance).map(str::to_owned),
        incident_carriers: Vec::new(),
        provenance: vec![CompilerProvenance::ObservedWeak],
    });
    id
}

fn compiler_edge_exists(
    program: &CandidateProgram,
    start_index: usize,
    end_index: usize,
    start: CompilerPoint2,
    end: CompilerPoint2,
    config: &DecodeConfig,
) -> bool {
    let tolerance = f64::from(config.vertex_merge_px / config.image_size.max(1) as f32).max(1e-6);
    program.edges.iter().any(|edge| {
        ...
        segments_match_with_tolerance(start, end, edge_start, edge_end, tolerance)
    })
}

fn segments_match_with_tolerance(
    left_start: CompilerPoint2,
    left_end: CompilerPoint2,
    right_start: CompilerPoint2,
    right_end: CompilerPoint2,
    tolerance: f64,
) -> bool {
    let forward = compiler_distance(left_start, right_start) <= tolerance
        && compiler_distance(left_end, right_end) <= tolerance;
    let reverse = compiler_distance(left_start, right_end) <= tolerance
        && compiler_distance(left_end, right_start) <= tolerance;
    forward || reverse
}

fn next_compiler_carrier_id(program: &CandidateProgram) -> usize {
    program
        .carriers
        .iter()
...
fn next_compiler_edge_id(program: &CandidateProgram) -> usize {
    program
        .edges
        .iter()
        .map(|edge| edge.id)
...
fn compiler_line_normal(start: CompilerPoint2, end: CompilerPoint2) -> CompilerPoint2 {
    let dx = end.x - start.x;
    let dy = end.y - start.y;
    let length = (dx * dx + dy * dy).sqrt().max(1e-12);
    CompilerPoint2::new(-dy / length, dx / length)
}

fn compiler_support_interval(start: CompilerPoint2, end: CompilerPoint2) -> [f64; 2] {
    let dx = end.x - start.x;
    let dy = end.y - start.y;
    let length = (dx * dx + dy * dy).sqrt().max(1e-12);
...
fn compiler_carrier_family(
    start: CompilerPoint2,
    end: CompilerPoint2,
    assignment: CompilerAssignmentLabel,
) -> CompilerCarrierFamily {
    if assignment == CompilerAssignmentLabel::Boundary {
        return CompilerCarrierFamily::Border;
    }
    let dx = (end.x - start.x).abs();
    let dy = (end.y - start.y).abs();
    if dy <= 1e-6 {
        CompilerCarrierFamily::Horizontal
    } else if dx <= 1e-6 {
        CompilerCarrierFamily::Vertical
    } else if (dx - dy).abs() <= 1e-3 {
        if (end.x - start.x) * (end.y - start.y) >= 0.0 {
            CompilerCarrierFamily::DiagonalPositive
        } else {
            CompilerCarrierFamily::DiagonalNegative
        }
    } else {
        CompilerCarrierFamily::Free
    }
}

fn compiler_assignment_from_u8(value: u8) -> CompilerAssignmentLabel {
    match value {
        0 => CompilerAssignmentLabel::Mountain,
        1 => CompilerAssignmentLabel::Valley,
        2 => CompilerAssignmentLabel::Boundary,
        _ => CompilerAssignmentLabel::Unknown,
    }
}

fn compiler_vertex_kind(point: CompilerPoint2, tolerance: f64) -> CompilerVertexKind {
    ...
    if on_vertical && on_horizontal {
        CompilerVertexKind::Corner
    } else if on_vertical || on_horizontal {
        CompilerVertexKind::Boundary
    } else {
        CompilerVertexKind::Interior
    }
}

fn compiler_boundary_side(point: CompilerPoint2, tolerance: f64) -> Option<&'static str> {
...
fn compiler_distance(left: CompilerPoint2, right: CompilerPoint2) -> f64 {
...
pub fn decode_dense_outputs(
    ...
    Ok(DecodedFold {
        ...
        report: DecodeReport {
            ...
            decoder_backend: DecoderBackend::LegacyV2,
...
fn quality_report_value(
    ...
    json!({
        "status": status,
        "decoder_backend": DecoderBackend::LegacyV2.id(),
        "warnings": warnings,
