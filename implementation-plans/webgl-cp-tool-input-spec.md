...
| Flat Foldable Line | VertexMakeAngularlyFlatFoldable | "for flat foldable line 1. There are little dots on the end of each option in oriedita which look nicer, can you add those 2. In oriedita, if you click on one of the lines, it hides the other options. In ori studio if you click on one of the lines, you get this error:" | 🔨 **candidate-preview built — pending Zach's validation.** Root cause: the kernel collapsed Oriedita's 3-step flow (vertex → pick candidate ray → pick destination crease) into 2 points, using `points[1]` for *both* candidate and destination — so a click on a candidate (not near an existing crease) threw `nearest line outside selection distance`, and there was no candidate-selection step. **Fix:** Kernel — `VertexMakeAngularlyFlatFoldable` now accepts 3 points (`point[1]`=candidate, `point[2]`=destination; legacy 2-point still works); preview shows all candidates during steps 1–2, then the selected candidate + resulting crease at step 3. Command-level test added. Frontend — new `'candidate'` `StepKind` snaps onto the nearest previewed candidate ray; registry → 3-point `['point','candidate','crease']` (+ 3rd `toolStep`); candidate endpoints render as dots (the "little dots" ask). **Both parity gaps now closed** (Zach asked): (1) a lone candidate auto-selects — the candidate step injects the single ray and advances, so it's 2 clicks not 3; (2) candidate-pick is distance-gated — a click that lands on no ray is ignored instead of committing the nearest one from across the canvas. |
| Extend Line | LengthenCrease | "For lengthen crease, clicking a line to select works, but in oriedita you can also draw a line that intersects with the line you are selecting to select it, which ori studio does not allow for" (then: "mark it as X and we can come back to it at the end") | ✅ **drag-to-select built + Zach validated.** The kernel already modelled Oriedita's LENGTHEN_CREASE_5 (a selection line + extension point), so this was a frontend gap: new `lengthen` input model driving a two-gesture canvas handler — drag the selection line across the crease(s), then click the target — committing 3 points `[selectionA, selectionB, extensionPoint]`. A click still works (zero-length line → the kernel's nearest-crease fallback). Crossed creases highlight in the selection style **on release** (Zach's call — not live under the cursor), computed frontend-side (`segmentsIntersect` / nearest-hit), matching the kernel's picked set (parallel-to-target filter only applies at extension, as in Oriedita). Panel prompt advances across the two gestures; commit sends the 3 points alone so an ambient selection can't hijack the kernel's line-id path. |
...
| Converging Lines | DrawCreaseAngleRestricted | "Okay yeah this one isn't quite right - when i try to select a line, it gets interpreted as the first point of two points. In oriedita you just select a line, no option to select two points" | 🔨 **dual-mode + candidate-point built — pending Zach's validation.** Matches Oriedita's `MouseHandlerDrawCreaseAngleRestricted`: **dual first click** — click a crease → its two endpoints are the base (no 2-point path), or click two points → base segment; then the angle-system **converging rays** appear from both endpoints and their **intersections** are candidate converge points (dots); pick one → two creases drawn base-endpoints→converge. **Impl:** kernel execute unchanged (base = points[0..1], converge = points[2]); preview now also draws the two result creases at the hovered converge point. Frontend — bespoke `feedConverging` handler in the canvas (dual first click + candidate-*point* snap), new `toolCommandPreviewPoints` channel rendered as dots, `webglActiveTool.converging` flag. **Deferred nicety:** angle-system divider is fixed at the default (4) — Oriedita reads it from the Angle System settings panel, not yet wired to WebGL. |
...
| Operation frame | OperationFrameCreate | "i don't really understand this one, can you explain how ti wroks and make sure this functionality actually exists in the app? it might not have been ported" (then, on testing: "our CP exporting code is different, so can you just hide the frame tool for now") | ⏭️ **built, then hidden (Zach).** Built the WebGL overlay (a dedicated dashed, screen-constant-width overlay-frame channel in the renderer, driven from the panel) **and** fixed the interaction — it was wired to `drag-path` (lasso), so it traced a freeform path instead of the rectangle it actually is; switched to `drag-box` (rubber-band rectangle, commits the two corners the kernel's `operation_frame_press/release` want). Then **hidden** (`placement: 'hidden-ui-only'`) because CP export is handled differently here. Overlay + create interaction stay in the code for when export grows a crop step. Move/resize of an existing frame is not ported (create only). |
...
| Measure length 1/2 | DisplayLengthBetweenPoints1/2 | "looks like some of these are not implemented" (then: "wait what don't hide all 5 - mark them as an X, i want them. don't hide anyhting unless i tell you to") | 🔨 **built (kernel-backed) — pending Zach's validation.** Non-mutating: the **preview** carries `measurement` (kernel `operations::measure`, exact Oriedita parity — no JS recompute) + a guide line the surface renders as a **screen-space dashed** stroke (new dash mode in `strokeProgram`). `handleWebglToolPreviewInput` surfaces the value into the panel slot live; `handleWebglToolCommit` short-circuits measure ops (never executes — there is no execute arm by design) and finalizes. Verified in-app: dashed guide + live L1 value. |
| Measure angle 1/2/3 | DisplayAngleBetweenThreePoints1/2/3 | "looks like some of these are not implemented" (then: "don't hide all 5 - mark them as an X, i want them") | 🔨 **built (kernel-backed) — pending Zach's validation.** Same path; angle from `angle_between_three_points` (vertex = 2nd point, Oriedita orientation) — the parity nuance the JS `computeCpMeasurementValue` risked. Two guide rays. |
...
| Check-fix (all 8) | Check1-4, CheckCamv, Fix1/2, FixInaccurate | "can we just mark all the checks / fixes as deferred, can't really test without overlay / markers" | ✅ **unblocked — overlay ported, render pipeline verified.** The WebGL diagnostics overlay is built (see Phase 6 below): `Check1–4` / `CheckCamv` pipe `diagnostic_entries` into the marker/stroke/wedge geometry the surface draws, and running a check renders its markers + auto-frames the issue. `Fix1/2` / `FixInaccurate` mutate through the kernel like any command. Left to Zach: end-to-end functional validation on a CP with real violations. |
...
| ✔ | Operation | steps | SVG ref | notes |
|---|---|---|---|---|
| ⏭️ | FlatFoldableCheck | — |  | Hidden (Zach) — `placement: 'hidden-ui-only'`. |
| ⏭️ | OperationFrameCreate | — |  | Built (overlay + `drag-box` create), then hidden (Zach) — export handled differently. Now `drag-box`, not `drag-path`. |
| ⬜ | SelectLasso | — |  |  |
| ⬜ | SelectPolygon | — |  |  |
| ⬜ | UnselectLasso | — |  |  |
| ⬜ | UnselectPolygon | — |  |  |
