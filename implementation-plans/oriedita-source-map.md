...
| `FoldingServiceImpl.fold` | Determine fold scope and start folding. | `oristudio-cp-wasm::folded_figure_fold`, workspace folded-figure state | 10 | WASM session entrypoint and stale-aware workspace list; selection scope, Oriedita folded-list replacement semantics, and async task dispatch are UI/service wiring |
...
| `FoldingServiceImpl.foldAnother` | Request another overlap solution. | `folding::fold_another`, `folding::WorkerOverlapEnumerator`, `oristudio-cp-wasm::folded_figure_fold_another`, workspace folded-figure state | 10 | Porting; pure session action, WASM session export, and workspace state update path; full Oriedita control wiring unsupported |
