...
| `folding steps refused` | as above plus `refusal_reason` (`non_rectangular`/`point_cap`/`budget`/`error`) | The breakdown produced no sequence. `non_rectangular` is D10's refusal (a hexagonal or open outline), `point_cap` the 600,000-point ceiling, `budget` the 30-second clock, `error` a worker failure. This is the row that says whether the V1 model's limits bite in practice |
