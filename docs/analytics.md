...
| `references mode changed` | `mode` (`find`/`sequence`) | The reader switched the References workspace between finding one reference and reading the folding sequence. The workspace lands in Find and plans only when asked, so `sequence` counts how often the whole sequence is wanted at all, against picks in Find |
| `references step jumped` | `target_kind` (`crease`/`vertex`) | In Sequence mode a tap on the sheet — a crease already made, or one still to come drawn as a ghost — moved the strip to the step that makes it. Whether the sheet is used as a way into a long sequence, which is what the ghosts exist for |
