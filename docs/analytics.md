...
| `view drawer opened` | `workspace`, `pane` | The touch-only View drawer is opened, on the side pane `pane` names (`cp-view-controls`, `cp-properties` or `simulator-view-controls`). It has no fine-pointer counterpart — the pane is docked there — so every one of these is a touch session going looking for the view options, which is the question undocking the pane raises. No menu action reaches it, so the `command invoked` chokepoint cannot see it |
...
| `folding steps refused` | as above plus `refusal_reason` (`non_rectangular`/`point_cap`/`budget`/`error`) | The breakdown produced no sequence. `non_rectangular` is D10's refusal (a hexagonal or open outline), `point_cap` the 600,000-point ceiling, `budget` a run ceiling — which the sequence no longer sets, so this value is a driver that did — and `error` a worker failure. This is the row that says whether the V1 model's limits bite in practice |
...
| `references approximation warning shown` | `inexact_steps_bucket`, `exactness_class` (`exact`/`snappable`/`off_lattice`) | The modal warning that a precreasing sequence contains approximated folds was shown — once per plan with a step that is not exact (folded by the closest construction there was, or sighted from one). A pattern out of Detect CP from Image that converged a hair off its lattice is the case it exists for, so against `folding steps completed` it is the share of sequences a reader has to verify by hand; `exactness_class` says which class of pattern earned it. Nothing about the folds |
