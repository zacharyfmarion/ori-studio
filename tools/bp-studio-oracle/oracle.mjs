...
async function optimizerSolveCommand(args) {
  ...
  const requestText =
    requestPath === '-' ? readFileSync(0, 'utf8') : readFileSync(requestPath, 'utf8');
...
function classify(path) {
  ...
  if (path === 'makefile' || path.startsWith('lib/optimizer/') || path.startsWith('lib/include/')) {
    return map(
      'optimizer oracle artifact',
      'optimizer::oracle',
      'Unsupported',
      'Oracle input only; Rust implementation must not depend on opaque artifacts.',
    );
  }
  ...
  if (path === 'src/shared/types/geometry.ts') {
    return map(
      ...
      'math::geometry, sweep::rr_intersection',
      'Unit-tested',
      'Rust exposes concrete point/path/arc-path structs instead of TS structural aliases.',
    );
  }
  if (path.startsWith('src/shared/types/')) {
    return map(
      'TypeScript type-only declaration',
      'Rust static types',
      'Documented-difference',
      'Compile-time TS declaration surface is represented by Rust types where behavior needs it.',
    );
  }
  if (path.startsWith('src/shared/utils/')) {
    ...
    if (path === 'src/shared/utils/clone.ts' || path === 'src/shared/utils/copy.ts') {
      return map(
        'shared structural clone/copy helper',
        'Rust Clone/serde ownership',
        'Documented-difference',
        'Rust uses typed Clone and serde-owned values instead of JS runtime object graph copying.',
      );
    }
  ...
  if (path.startsWith('src/shared/polyfill/') || path.startsWith('src/shared/frontend/')) {
    return map(
      ...
      'oristudio-bp-wasm or app integration',
      'Out-of-scope-ui',
    );
  }
  if (path === 'src/core/math/sweepLine/classes/chainer/chainer.ts') {
    return map(
      'sweep-line path chainer',
      'sweep::{aa_union,general_union}',
      'Unit-tested',
      'Covered through AAUnion exact path reconstruction and GeneralUnion epsilon path reconstruction.',
    );
  }
  if (path === 'src/core/math/sweepLine/classes/chainer/unionChainer.ts') {
    return map(
      'sweep-line source-tracking chainer',
      'sweep::rough_union',
      'Unit-tested',
      'Covered through RoughUnion source mapping.',
    );
  }
  if (path === 'src/core/math/sweepLine/classes/chainer/arcChainer.ts') {
    return map(
      'sweep-line arc path chainer',
      'sweep::rr_intersection',
      'Unit-tested',
      'Covered through RRIntersection arc-path reconstruction.',
    );
  }
  ...
  if (path === 'src/core/math/sweepLine/classes/segment/lineSegment.ts') {
    return map(
      'general sweep segment',
      'sweep::{clip,overlap,general_union}',
      'Unit-tested',
      'Covered through clipping, overlap, and GeneralUnion subdivision tests.',
    );
  }
  if (path === 'src/core/math/sweepLine/classes/segment/arcSegment.ts') {
    return map(
      ...
      'sweep::rr_intersection',
      'Unit-tested',
      'Covered through RRIntersection intersection and trisection tests.',
    );
  }
  if (isSweepScaffolding(path)) {
    return map(
      'sweep-line shared scaffolding',
      'sweep concrete operation modules',
      'Documented-difference',
      'Rust ports the shared TS class hierarchy into concrete sweep modules; Stacking, Clip, Overlap, AAUnion, RoughUnion, GeneralUnion, and RRIntersection are unit-tested.',
    );
  }
  if (path === 'src/core/math/sweepLine/stacking/stacking.ts') {
    return map(
      ...
      'sweep::stacking',
      'Unit-tested',
      'General sweep subset for non-intersecting path stacking is ported; clip/polybool/intersectors remain unsupported.',
    );
  }
  if (path === 'src/core/math/sweepLine/clip/clip.ts') {
    return map(
      'CP clipping and subdivision',
      'sweep::clip',
      'Unit-tested',
      'Manual oracle confirmed upstream spec counts and duplicate-line behavior.',
    );
  }
  if (
    path === 'src/core/math/sweepLine/clip/overlap.ts' ||
    path === 'src/core/math/sweepLine/clip/overlapIntersector.ts'
  ) {
    return map(
      'sweep-line overlap detection',
      'sweep::overlap',
      'Unit-tested',
      "Preserves BP Studio's false result for full containment.",
    );
  }
  if (path === 'src/core/math/sweepLine/polyBool/aaUnion/roughUnion.ts') {
    return map(
      ...
      'sweep::rough_union',
      'Unit-tested',
      'Upstream RoughUnion spec cases ported.',
    );
  }
  if (
    path.startsWith('src/core/math/sweepLine/polyBool/aaUnion/') &&
    path !== 'src/core/math/sweepLine/polyBool/aaUnion/roughUnion.ts'
  ) {
    return map(
      'axis-aligned polygon union',
      'sweep::aa_union',
      'Unit-tested',
      'Full upstream AAUnion spec cases ported.',
    );
  }
  if (path.startsWith('src/core/math/sweepLine/polyBool/generalUnion/')) {
    return map(
      'general polygon union',
      'sweep::general_union',
      'Unit-tested',
      'Upstream GeneralUnion spec cases ported, including floating-error regressions.',
    );
  }
  if (path.startsWith('src/core/math/sweepLine/polyBool/rrIntersection/')) {
    return map(
      ...
      'sweep::rr_intersection',
      'Unit-tested',
      'Upstream RRIntersection spec cases ported, including arc trisection and epsilon regressions.',
    );
  }
  ...
  if (path === 'src/core/math/invalidParameterError.ts') {
    return map(
      'fraction invalid-parameter error',
      'math::rational, BpError::InvalidInput',
      'Unit-tested',
      'Rust represents the thrown JS error as a typed Result error.',
    );
  }
  ...
  if (path.startsWith('src/core/design/context/')) {
    if (
      ...
      return map(
        'optimizer hierarchy area tree',
        'tree::AreaTree',
        'Unit-tested',
        'Translated upstream AreaTree specs cover simplification, area balancing, and hierarchy expansion.',
      );
    }
    if (
      ...
      return map(
        'tree AABB propagation',
        'tree::Aabb',
        'Unit-tested',
        'Covered through translated upstream tree AABB specs.',
      );
    }
    if (
      ...
      return map(
        ...
        'tree::BpTree',
        'Unit-tested',
        'Covers construction, root balancing, editing, BFS JSON, distance records, and distance maps.',
      );
    }
    if (path === 'src/core/design/context/index.d.ts') {
      return map(
        'tree/context TypeScript declarations',
        'Rust tree/model/static types',
        'Documented-difference',
        'Rust exposes concrete structs instead of TS declaration interfaces.',
      );
    }
  ...
  if (
    ...
    return map(
      'tree maintenance task',
      'tree::BpTree::recompute',
      'Unit-tested',
      'Task behavior is embedded in BpTree recompute for the headless kernel; generic processor scheduling is ported separately.',
    );
  }
  if (path === 'src/core/design/tasks/invalidJunction.ts') {
    return map(
      'invalid junction update task',
      'layout::InvalidJunction plus engine task TODO',
      'Porting',
      'Invalid junction polygon calculation is ported; State diff/update-result task wiring remains unsupported until layout task integration.',
    );
  }
  if (path === 'src/core/design/tasks/stretch.ts') {
    return map(
      'stretch grouping task',
      'layout::{group_junctions, uncovered_junction_indices, LayoutStretch}',
      'Porting',
      'Quadrant union grouping, uncovered-junction filtering, geometric covering checks, and stretch cache/update shell are ported; State diff/update-result integration and pattern task scheduling remain unsupported.',
    );
  }
  ...
  if (path.startsWith('src/core/design/layout/')) {
    if (path === 'src/core/design/layout/store.ts') {
      return map(
        ...
        'layout::Store',
        'Unit-tested',
        'Rust preserves lazy next/rest progression, done state, and cached entries.',
      );
    }
    if (path === 'src/core/design/layout/junction/validJunction.ts') {
      return map(
        'valid junction model',
        'layout::ValidJunction',
        'Unit-tested',
        'Covers oriented JSON/signature shape, covering/practical-covering semantics, involvement, closer-than comparison, and base rectangle construction.',
      );
    }
    if (
      path === 'src/core/design/layout/junction/junction.ts' ||
      path === 'src/core/design/layout/junction/invalidJunction.ts'
    ) {
      return map(
        'junction creation and invalid overlap model',
        'layout::{create_junction, InvalidJunction}',
        'Unit-tested',
        'Covers valid-vs-invalid branch selection, canonical id ordering, invalid rounded-rectangle polygon calculation, and processed-state update.',
      );
    }
    if (path === 'src/core/design/layout/nodeSet.ts') {
      return map(
        ...
        'layout::NodeSet',
        'Unit-tested',
        'Covers leaf collection, quadrant coverage, LCA caching, distance triples, and length-change comparison over BpTree.',
      );
    }
    if (path === 'src/core/design/layout/repository.ts') {
      return map(
        ...
        'Porting',
        'Signature/origin/factor capture, oriented junction JSON, quadrant grouping, directional ordering, opposite-map creation, NodeSet validity checks, serialized repo preservation, and unsupported config-search boundary are ported; generated configuration stores and joiner cache remain unsupported.',
      );
    }
    if (path === 'src/core/design/layout/stretch.ts') {
      return map(
        'stretch lifecycle shell',
        'layout::LayoutStretch',
        'Porting',
        'Construction, same-signature NodeSet refresh, origin updates, drag-time repository caching/reuse, cleanup, JSON shell, and unsupported completion boundary are ported; pattern completion remains unsupported.',
      );
    }
    if (path === 'src/core/design/layout/configuration.ts') {
      return map(
        'configuration data shell',
        'layout::LayoutConfiguration',
        'Porting',
        'Raw-partition cleanup, flat overlap map, signature/session serialization, and unsupported pattern-generation boundary are ported; pattern stores, free corners, and side diagonals remain unsupported.',
      );
    }
    if (path === 'src/core/design/layout/partition.ts') {
      return map(
        'partition data shell',
        'layout::LayoutPartition',
        'Porting',
        'Corner maps, constraint/external filters, flap lookup, exposed-overlap trimming, and resolve-division behavior are ported; displacement solving, external target wiring, and pattern devices remain unsupported.',
      );
    }
    if (path === 'src/core/design/layout/generators/configGeneratorContext.ts') {
      return map(
        'configuration generator context',
        'layout::generators::ConfigGeneratorContext',
        'Unit-tested',
        'Temporary overlap id allocation, junction-to-overlap conversion, x/y cuts, raw single-mode make, cleanup make, and explicit config-search unsupported boundary are ported with direct Rust tests.',
      );
    }
    if (path === 'src/core/design/layout/generators/searchUtils/relay.ts') {
      return map(
        'relay search utility',
        'layout::generators::search_relay',
        'Unit-tested',
        'Oriented/non-oriented x/y relay partition rewrites, split filtering, strategy-order variants, intersection/socket/internal corner rewrites, and relay shifts are ported with direct Rust tests.',
      );
    }
    if (path === 'src/core/design/layout/generators/searchUtils/splitJoin.ts') {
      return map(
        'split-join search helpers',
        'layout::generators::{to_split_items, get_exposed_part}',
        'Unit-tested',
        "Cover checks, raw split-item extraction, horizontal split detection, exposed remaining-part shrinkage, intersection replacement, and BP's join-partition mutation side effect are ported with direct Rust tests.",
      );
    }
    if (path === 'src/core/design/layout/generators/deviceGenerator.ts') {
      return map(
        'single-overlap device generator',
        'layout::generators::device_generator',
        'Porting',
        "Single-overlap GOPS, Kamiya half-integral plus BP's fallthrough to GOPS, universal GPS scaling/reverse/slack, DTO conversion, and explicit join/general unsupported boundaries are ported with direct Rust tests.",
      );
    }
    if (path === 'src/core/design/layout/pattern/quadrant.ts') {
      return map(
        ...
        'layout::Quadrant',
        'Porting',
        'Factor/weight ordering, start/end points, corner lookup, containment validity checks, and shared helpers are ported; overlap-corner targeting and trace-specific helpers remain for later pattern/trace stages.',
      );
    }
    if (path === 'src/core/design/layout/pattern/region.ts') {
      return map(
        'pattern region axis parallels',
        'layout::pattern::axis_parallels',
        'Unit-tested',
        "Axis-parallel crease generation over exact region contours is ported with direct Rust tests; BP's TODO about improving intersection handling remains preserved.",
      );
    }
    if (path === 'src/core/design/layout/pattern/piece.ts') {
      return map(
        'pattern piece geometry',
        'layout::pattern::PatternPiece',
        'Unit-tested',
        'SCR dimensions, offset handling, detour substitution including corner-start behavior, anchors, direction, contour, reverse, shrink, and detour mutation are ported with direct Rust tests.',
      );
    }
    if (path === 'src/core/design/layout/pattern/addOn.ts') {
      return map(
        'pattern add-on region',
        'layout::pattern::PatternAddOn',
        'Unit-tested',
        'Add-on contour/ridge conversion, reduced direction, and axis-parallel region behavior are ported with direct Rust tests.',
      );
    }
    if (path === 'src/core/design/layout/pattern/gadget.ts') {
      return map(
        'pattern gadget geometry',
        'layout::pattern::PatternGadget',
        'Porting',
        'Piece wrapping, sparse anchors, spans, slacks, contour joining, reverse GPS, ray intersection, signature simplification, and connection-slack shell are ported; connection-slack needs joiner/device integration coverage.',
      );
    }
    if (path === 'src/core/design/layout/pattern/device.ts') {
      return map(
        'pattern device shell',
        'layout::pattern::PatternDevice',
        'Porting',
        'Device construction, JSON offset/add-on behavior, local region collection, local contours, local axis-parallels, local inner ridges, and signature simplification are ported; positioned anchors, draw/trace ridges, dragging ranges, connection ridges, and neighbor cache remain unsupported.',
      );
    }
    if (path === 'src/core/design/layout/pattern/pattern.ts') {
      return map(
        'seeded pattern shell',
        'layout::pattern::LayoutPattern',
        'Porting',
        'Seeded pattern construction, flattened gadget access, JSON serialization, and origin-dirty shell are ported; unseeded positioning, connection targets, moved-device state, and positioner dispatch remain unsupported.',
      );
    }
  ...
  if (path === 'src/core/service/updateModel.d.ts' || path === 'src/core/service/updateResult.ts') {
    return map(
      'update payload and accumulator',
      'engine::update',
      'Unit-tested',
      'Rust preserves the empty UpdateModel JSON shape, ordered record assignment semantics, tree export payloads, and flush reset behavior.',
    );
  }
  if (path === 'src/core/service/processor.ts') {
    return map(
      ...
      'engine::processor',
      'Unit-tested',
      "Rust preserves BP Studio's dependant-task priority calculation, heap-set de-duplication by task identity, and state reset after successful runs.",
    );
  }
  if (path === 'src/core/service/state.ts') {
    return map(
      ...
      'engine::state',
      'Unit-tested',
      'Persistent/semi-persistent/temporary container reset semantics are ported; layout-specific payload algorithms remain in later stages.',
    );
  }
  if (path === 'src/core/controller/treeController.ts') {
    return map(
      'tree controller API',
      'engine::BpSession tree commands',
      'Unit-tested',
      'Tree edit, leaf add/remove, join/split/merge, child-id lookup, and hierarchy request surfaces are ported with Rust command-trace tests.',
    );
  }
  if (path === 'src/core/controller/designController.ts') {
    return map(
      'design controller API',
      'engine::BpSession design init/update',
      'Porting',
      'Tree-bearing init/update surface is ported; downstream junction/stretch/pattern/graphics tasks remain unsupported until layout stages.',
    );
  }
  if (
    path.startsWith('src/core/controller/') ||
    path.startsWith('src/core/service/') ||
    path.startsWith('src/core/routes/') ||
  ...
  if (
    path.startsWith('src/client/project/changes/commands/') ||
    path === 'src/client/project/changes/step.ts'
  ) {
    return map(
      'history command/step semantics',
      'engine::history',
      'Unit-tested',
      'Command JSON, signatures, coalescing, memento cancellation, step serialization, and partial-result truncation are covered in Rust tests.',
    );
  }
  if (path === 'src/client/project/changes/history.ts') {
    return map(
      ...
      'Porting',
      'Queue/flush/max-depth/save-index/navigation truncation semantics are ported; project-object mutation and UI selection restoration still need session/frontend wiring.',
    );
  }
  ...
  if (path.startsWith('src/client/project/components/grid/')) {
    return map(
      'grid/sheet constraints',
      'grid',
      'Unit-tested',
      'Pure rectangular/diagonal constraints, dimensions, transforms, border/grid payloads, and resize shift decisions are covered in Rust tests; Vue/Pixi drawing is out of scope.',
    );
  }
  if (path === 'src/client/project/components/sheet.ts') {
    return map(
      'sheet grid transform helpers',
      'grid',
      'Porting',
      'Subdivide/rotate/flip matrices, relative-point scaling, and image dimension math are ported; viewport, Pixi layers, selection, and draw lifecycle are out of scope for this headless stage.',
    );
  }
  ...
  if (
    path.startsWith('src/app/services/importService.ts') ||
    path.startsWith('src/app/services/exportService.ts')
  ) {
    return map('file import/export service', 'io', 'Unit-tested');
  }
  if (
    path.startsWith('src/app/') ||
    path.startsWith('src/client/screen/') ||
    path.startsWith('src/client/controllers/') ||
    path.startsWith('src/locale/') ||
    path.startsWith('src/public/') ||
    path.startsWith('src/other/') ||
    path.startsWith('src/log/')
  ) {
    return map('presentation/UI/PWA asset', 'app integration later', 'Out-of-scope-ui');
  }
  if (
    path.startsWith('src/client/svg/') ||
    path.startsWith('src/client/utils/') ||
    path.startsWith('src/client/shared/') ||
    path.startsWith('src/client/base/') ||
    path.startsWith('src/client/services/') ||
    path === 'src/client/main.ts' ||
    path === 'src/client/options.ts'
  ) {
