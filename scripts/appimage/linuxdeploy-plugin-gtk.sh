...
# ==============================================================================
# Ori Studio local change — everything below this line is ours.
# ==============================================================================
#
# Leave GLib to the host.
#
# Upstream force-deploys libgio and libgobject (the FIND_ARRAY above), and
# linuxdeploy pulls libglib and libgmodule in behind them. The AppImage
# excludelist does not name any of the four, so they also arrive as ordinary
# dependencies of the app binary — which is why removing them from FIND_ARRAY
# would not be enough, and why this sweep is a deletion rather than an
# omission.
#
# What that costs users: the bundled copy is whatever the build runner had
# (GLib 2.72.4 on ubuntu-22.04), and it wins over the host's for every symbol.
# But it does not stop the host's GIO modules from being scanned — the hook
# above sets GIO_EXTRA_MODULES, never GIO_MODULE_DIR, so libgio still reads its
# compiled-in /usr/lib/<arch>/gio/modules. On any host newer than the runner
# those modules were built against a newer GLib, and every one of them fails to
# load:
#
#   /usr/lib/x86_64-linux-gnu/gvfs/libgvfscommon.so: undefined symbol: g_task_set_static_name
#
# Deleting the bundled copy hands all of it back to the host. It is safe in the
# other direction because GLib does not remove symbols: the 2,559 GLib symbols
# the rest of this AppDir imports are all present in 2.80 and 2.88, and the
# oldest host we support is the runner's own 2.72.
#
# Deleting works because of linuxdeploy's ordering, which is deliberate on its
# side: it flushes its deferred copies *before* running input plugins ("to make
# sure all files the plugins might expect" are there — src/main.cpp), and after
# them it only deploys icons and desktop files. Nothing re-resolves
# dependencies, so what we remove here stays removed, and the output plugin
# packs the AppDir as we leave it. Upstream's libwebkit patch just above relies
# on the same ordering.
#
# scripts/verify-appimage-bundle.mjs asserts the result on the built artifact,
# so if that ever changes the release fails rather than quietly shipping GLib
# again.
#
# See implementation-plans/appimage-host-glib.md.

echo "Removing bundled GLib so the host's GIO modules can load"
for soname in libglib-2.0.so libgobject-2.0.so libgio-2.0.so libgmodule-2.0.so libgthread-2.0.so; do
    find "$APPDIR" -name "${soname}*" -print -delete
done
