...
def main():
    ...
    for r in read(a.proposals/'runs.json'):
        ...
        before=read(Path(fallbacks[key]['prediction']));fold=read(Path(r['prediction']))
        chosen=dict(r);accepted=False
        if before['vertices_coords']==fold['vertices_coords']:
            accepted=True;report={'unchanged':True,'accepted':True,'seconds':0.}
        else:
            ...
            with (out/'judge.log').open('w') as log:
                subprocess.run([str(a.binary.resolve()),str(inp),str(out/'judge'),
            ...
            report={'seconds':time.monotonic()-start,'accepted':accepted,'status':solved['status'],
