...
publish_release() {
    ...
    echo "  2. Attach the updater manifest — nothing in CI does this:"
    echo "       ./scripts/publish-updater-manifest.sh $version"
    echo "  3. Install it and confirm it launches"
    echo "  4. Arm it:  gh release edit $tag_name --prerelease=false"
    echo ""
    echo "Step 2 only runs between steps 1 and 4, and skipping it strands users:"
    echo "releases/latest ends up with no latest.json, so the updater endpoint"
    echo "404s and every existing install silently stops seeing updates."
    echo ""
    echo "Until step 4, releases/latest still points at the previous release, so"
